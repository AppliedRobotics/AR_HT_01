...
def jstatecb(jointstates):
    ...
    if delta >=0.02:
        ...
        odom_broadcaster.sendTransform((-0.231,0,0.3), laser_quat, time, "laser", "base_footprint")
        # camera_quat = tf.transformations.quaternion_from_euler(0,0,-pi/2)
        # odom_broadcaster.sendTransform((-0.251,0,0.16), camera_quat, time, "camera_rgb_optical_frame", "base_footprint") 
...
def listeners():
    ...
    rospy.Subscriber("joint_states", JointState, jstatecb)
    rospy.spin()
    rate =rospy.Rate(50)
